// Hitster Webapp – Spotify PKCE + QR Scan (iPhone Safari)
...
// GitHub Pages: ensure callback points to /callback.html in same folder
...
let currentFacingMode = "environment"; // try environment first
let mirrorPreview = false; // can be toggled to match camera orientation on some devices
...
function setLoginUI() {
  if (accessToken) {
    loginState.textContent = "verbunden ✅";
    btnLogin.textContent = "Re-Login";
    hintAuth.textContent = "Spotify verbunden ✅";
  } else {
    loginState.textContent = "nicht verbunden";
    btnLogin.textContent = "Spotify Login";
    hintAuth.textContent = "Tippe auf „Spotify Login“, dann „JETZT SPIELEN“.";
  }
}
...
async function ensureDeviceAndPlay(trackId){
  ...
  // Transfer playback to device (helps on iPhone)
...
async function startScanner(){
  ...
  // Request higher resolution for better decoding
  const constraints = {
    video: {
...
async function scanLoop(){
  ...
  // Throttle to reduce CPU and improve stability
  if (now - lastScanTs < 120) {
    requestAnimationFrame(scanLoop);
    return;
  }
  lastScanTs = now;
  ...
  try {
    // Prefer native BarcodeDetector when available (more reliable on some iOS versions)
    if (detector) {
...
async function onTrackDetected(trackId){
  ...
  if (!accessToken) {
    alert("Bitte erst oben „Spotify Login“ drücken.");
    show(screenStart);
    return;
  }
...
btnStart.addEventListener("click", async () => {
  loadTokens();
  if (!accessToken) {
    alert("Bitte erst „Spotify Login“ drücken.");
    return;
  }
  try {
    await startScanner();
...
btnSwitchCam.addEventListener("click", async () => {
  // Switch between environment/user + toggle mirror for preview
  stopScanner();
  currentFacingMode = (currentFacingMode === "environment") ? "user" : "environment";
  mirrorPreview = (currentFacingMode === "user"); // front cam is usually mirrored
  try {
    await startScanner();
  } catch(e) {
    alert("Kamera wechseln ging nicht.");
  }
});

video.addEventListener("dblclick", () => {
  // quick mirror toggle if QR appears "wrong"
...
(async () => {
  // If returned from callback, exchange code
  const code = sessionStorage.getItem("spotify_auth_code");
  if (code && !accessToken) {
    sessionStorage.removeItem("spotify_auth_code");
    try {
