...
// --- Safety bootstrap: overwrite fallback start handler ASAP ---
try{
  window.__startGame = function(){
    // never reference undeclared identifiers here (GitHub Pages should always load this safely)
    try{ if(window.FLOW) window.FLOW.setStarted(true); }catch{}
    try{ document.getElementById("startScreen")?.classList.add("isLeaving"); }catch{}
    try{ setTimeout(()=>document.getElementById("startScreen")?.classList.add("isHidden"), 220); }catch{}
    try{ document.getElementById("appShell")?.classList.remove("isHidden"); }catch{}
    try{
      const token = localStorage.getItem("token");
      if(!token) setTimeout(()=>login(), 120);
    }catch{}
  };
}catch{}
...
// Global fallback (also used by inline onclick in HTML)
window.__startGame = function(){
...
document.getElementById("btnStart")?.addEventListener("click", () => window.__startGame?.());
document.getElementById("btnToSettings")?.addEventListener("click", () => window.__startGame?.());
const token = localStorage.getItem("token");
  if(!token){
    setTimeout(()=>login(), 120);
  }
...
// Fallback: if something prevents direct handler binding, still react to the start buttons.
document.addEventListener("click", (e)=>{
  const t = e.target;
  if(!(t instanceof Element)) return;
  if(t.id === "btnStart" || t.id === "btnToSettings"){
    // if direct handlers already ran, this is harmless
    window.__startGame?.();
  }
});
