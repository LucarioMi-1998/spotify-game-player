    ...
    const onScan = (data)=>{
      (async ()=>{
        try{
          ...
          // QR ist gültig → Scanner sofort schließen und Player sofort zeigen.
          // Wichtig: UI darf NICHT davon abhängen, ob Spotify Playback sofort klappt.
          stopQRScanner();
          // Extra-sicher: Overlay wirklich verstecken (falls Safari das DOM-Update verzögert)
          document.getElementById("camBox")?.classList.add("hidden");
          setCamToast("");

          // Player-UI sofort sichtbar machen
          showApp();
          document.getElementById("playerCard")?.scrollIntoView({ behavior:"smooth", block:"start" });
          status("Track wird geladen…");

          // Track-Link für Fallback („In Spotify öffnen“)
          const openLink = document.getElementById("spotifyLink");
          if(openLink) openLink.href = "https://open.spotify.com/track/" + id;

          // Best-effort Playback: wenn Spotify nicht spielen kann, bleibt UI trotzdem aktiv.
          try{
            await play(id);
            try{ await player?.resume(); }catch{}
            status("Spielt");
          }catch(e){
            const msg = e?.message || "Spotify konnte nicht starten";
            status(msg);
            // Scanner bleibt zu – Nutzer kann über "Nächste Karte" erneut scannen.
          }
        }catch(e){
