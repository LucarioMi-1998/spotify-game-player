...
btnOpenSpotify?.addEventListener("click", () => {
  // Best effort: try to open Spotify app
  try { window.location.href = "spotify://"; } catch {}
  setStatus("Spotify geöffnet? Starte kurz einen Song in der Spotify-App und komm dann zurück.");
});
...
btnDebugToggle?.addEventListener("click", async () => {
  toggleDebug();
  try{
    const j = await apiFetch("https://api.spotify.com/v1/me/player/devices", { method:"GET", headers:{ "Content-Type":"application/json" }});
    const devices = (j?.devices||[]).map(d => ({
      name:d.name, type:d.type, is_active:d.is_active, is_private_session:d.is_private_session, id: d.id ? (d.id.slice(0,6)+"…") : null, volume_percent:d.volume_percent
    }));
    setDebug("DEVICES\n" + JSON.stringify(devices, null, 2));
  }catch(e){
    setDebug("DEVICES ERROR\n" + (e?.message||String(e)));
  }
});
...
async function connectPlayTrack(trackId) {
  ...
  if (!dev) {
    const err = new Error("Kein aktives Spotify-Gerät gefunden. Öffne Spotify → starte kurz einen Song.");
    err.status = 404;
    throw err;
  }
