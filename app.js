...
async function openScanner(){
  ...
  $("btnStopScan") && ($("btnStopScan").disabled = false);
  ...
  const onScan = (data) => {
    (async () => {
      try{
        ...
        if (!id){
          setCamToast("Ungültiger QR-Code");
          setTimeout(() => window.resumeQRScanner?.(onScan), 650);
          return;
        }

        // Scanner soll nach gültigem Scan weggehen
        stopQRScanner();
        hideScanner();
...
function closeScanner(){
  stopQRScanner();
  hideScanner();
  status("Scanner geschlossen");
  $("btnScan") && ($("btnScan").disabled = false);
  $("btnStopScan") && ($("btnStopScan").disabled = true);
}
...
async function init(){
  ...
  // Start button: ALWAYS triggers Spotify auth if not logged in
  ...
  // If we came back from login (postLogin=startGame), open scanner automatically
  ...
  if (post === "startGame"){
    showApp();
    // slight delay so CSS transition finishes before camera permissions sheet
