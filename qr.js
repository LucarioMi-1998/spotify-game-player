...
function scanLoop(video, onResult) {
  ...
  if (code?.data) {
    // Pause scanning to avoid multiple hits.
    // Keep the camera preview open so app.js can show feedback
    // (and decide whether to stop or continue scanning).
    scanning = false;
...
// Continue scanning without requesting the camera again.
function resumeQRScanner(onResult){
  const video = document.getElementById("camVideo");
  if(!video) return;
  scanning = true;
  scanLoop(video, onResult);
...
window.resumeQRScanner = resumeQRScanner;
